...
function computeOptimalPath(s, t, state) {
    ...
    //Removed number validation as we're dealing with objects now 
...
function computeOptimalPathWithWindow(s, t, w, state) {
    ...
    // Removed checking for valid numbers as we're using objects
    // validateComputeParameters(s, t);
    var start = new Date().getTime();
    state.m = s.length;
    state.n = t.length;
...
function retrieveOptimalPath(state) {
    ...
    while ((x > 0) || (y > 0)) {
        if ((x > 0) && (y > 0)) {
            var min = Math.min(
                state.distanceCostMatrix[x - 1][y],          // Insertion
                state.distanceCostMatrix[x][y - 1],          // Deletion
                state.distanceCostMatrix[x - 1][y - 1]);     // Match
            if (comparison.nearlyEqual(min, state.distanceCostMatrix[x - 1][y - 1], epsilon)) {
                x--;
                y--;
            } else if (comparison.nearlyEqual(min, state.distanceCostMatrix[x - 1][y], epsilon)) {
                x--;
            } else if (comparison.nearlyEqual(min, state.distanceCostMatrix[x][y - 1], epsilon)) {
                y--;
            }
        } else if ((x > 0) && (y === 0)) {
            x--;
